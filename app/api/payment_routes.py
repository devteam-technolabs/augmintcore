...
from app.models.user import Subscription, Transaction, User
from app.schemas.user import CheckoutSessionResponse, CheckoutSessionSchemas
...
async def create_checkout_session(
    ...
    return {"checkout_url": checkout_url, "status_code": 200}


@router.post("/cancel-subscription")
async def cancel_subscription(
    db: AsyncSession = Depends(get_async_session),
    current_user=Depends(auth_user.get_current_user),
):
    user = current_user
    user_id = user.id
    try:
        subscription = await payment_service.handle_cancel_subscription_at_period_end(
            db, user_id
        )
        return {"status_code": 200, "message": "Subscription cancelled successfully"}
...
async def stripe_webhook(request: Request):
    ...
    try:
        event = payment_service.construct_event(
    ...
    if event_type == "checkout.session.completed":
        ...
        async with AsyncSessionLocal() as session:
            await payment_service.handle_subscription(
                user_id,
                session,
                plan_name,
                plan_type,
    ...
    # Auto -Renewal
        ...
        if billing_reason == "recurring":
            try:
                ...
                async with AsyncSessionLocal() as session:
                    subscription = await payment_service.handle_auto_renewal(
                        db=session, event_data=event_data, event_id=event["id"]
                    )
    ...
    # SUBSCRIPTION DELETD PERMANENTLY
        ...
        try:
            ...
            async with AsyncSessionLocal() as session:
                await payment_service.handle_subscription_deleted(
            ...
            async with AsyncSessionLocal() as session:
                ...
                if subscription:
                    if cancel_at_period_end:
                        print(
                            f"[WEBHOOK] Subscription {subscription_id} scheduled for cancellation"
                        )
                    else:
